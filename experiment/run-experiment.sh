...
for ROOT in $(head -n $NRT "$DDIR/kron-$S/kron-${S}-roots.v"); do
	"$GAPDIR"/bfs -r $ROOT -f "$DDIR/kron-$S/kron-${S}.el" -n 1 -s
done
...
"$GRAPHBIGDIR/benchmark/bench_BFS/bfs" --dataset "$DDIR/kron-$S/kron-${S}" --rootfile "$DDIR/kron-$S/kron-${S}-${NRT}roots.v" --threadnum $OMP_NUM_THREADS

# Run the GraphMat BFS
for ROOT in $(head -n $NRT "$DDIR/kron-$S/kron-${S}-roots.1v"); do
	echo "BFS root: $ROOT"
	"$GRAPHMATDIR/bin/BFS" "$DDIR/kron-$S/kron-${S}.graphmat" $ROOT
done

# Run the GAP SSSP for each root
for ROOT in $(head -n $NRT "$DDIR/kron-$S/kron-${S}-roots.v"); do
	"$GAPDIR"/sssp -r $ROOT -f "$DDIR/kron-$S/kron-${S}.el" -n 1 -s
done

# Run the GraphBIG SSSP
# for ROOT in $(head -n $NRT "$DDIR/kron-$S/kron-${S}-roots.v"); do
# 	"$GRAPHBIGDIR/benchmark/bench_shortestPath/sssp" --dataset "$DDIR/kron-$S/kron-${S}" --rootfile "$DDIR/kron-$S/kron-${S}-${NRT}roots.v" --threadnum $OMP_NUM_THREADS
# done
"$GRAPHBIGDIR/benchmark/bench_shortestPath/sssp" --dataset "$DDIR/kron-$S/kron-${S}" --rootfile "$DDIR/kron-$S/kron-${S}-${NRT}roots.v" --threadnum $OMP_NUM_THREADS
...
for ROOT in $(head -n $NRT "$DDIR/kron-$S/kron-${S}-roots.1v"); do
	echo "SSSP root: $ROOT"
	"$GRAPHMATDIR/bin/SSSP" "$DDIR/kron-$S/kron-${S}.graphmat" $ROOT
done
...
for ROOT in $(head -n $NRT "$DDIR/kron-$S/kron-${S}-roots.v"); do
	"$POWERGRAPHDIR/release/toolkits/graph_analytics/sssp" --graph "$DDIR/kron-$S/kron-${S}.el" --format tsv --source $ROOT
done
...
for ROOT in $(head -n $NRT "$DDIR/kron-$S/kron-${S}-roots.v"); do
	"$GAPDIR"/pr -f "$DDIR/kron-$S/kron-${S}.el" -i $MAXITER -t $TOL -n 1
done
...
for ROOT in $(head -n $NRT "$DDIR/kron-$S/kron-${S}-roots.v"); do
	"$GRAPHBIGDIR/benchmark/bench_pageRank/pagerank" --dataset "$DDIR/kron-$S/kron-${S}" --maxiter $MAXITER --quad $TOL --threadnum $OMP_NUM_THREADS
done
...
for ROOT in $(head -n $NRT "$DDIR/kron-$S/kron-${S}-roots.1v"); do
	"$GRAPHMATDIR/bin/PageRank" "$DDIR/kron-$S/kron-${S}.graphmat"
done

# Run PowerGraph PageRank
for ROOT in $(head -n $NRT "$DDIR/kron-$S/kron-${S}-roots.v"); do
	"$POWERGRAPHDIR/release/toolkits/graph_analytics/pagerank" --graph "$DDIR/kron-$S/kron-${S}.el" --tol "$TOL" --format tsv
done
