...
Nai et al.~\cite{Nai:2016:architectural} provide a detailed performance analysis using GraphBIG as their reference implementation, profile Intel hardware counters, and measure the bottlenecks to measure scalability, computational efficiency for a variety of datasets.
...
The libraries are stable forks of the given repositories which are configured to ensure the experiments execute in the same manner. Similarly, multiple copies of the datasets are stored to ensure the datasets are formatted correctly. For example, the roots for BFS are selected such that they are part of a nontrivial connected component.
...
We refer to a graph's \emph{scale} when describing the size of the graph. Specifically, a graph with scale $S$ has $2^S$ vertices. For example, many of our experiments were run with $2^{22} = 4,194,304$ vertices and an average of 16 edges per vertex. The datasets are described further in Sect.~\ref{sec:datasets}. We measure strong scaling and speedup by varying the number of threads from one to the total number of threads on our server, 72.
...
The example workflow provided in our implementation yields the figures generated in this paper.
