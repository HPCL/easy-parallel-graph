...
\begin{table}
	\caption{A tabulated sample run of Graphalytics with 32 threads. Just one run per experiment is performed and these times do not take into account the different phases of execution. Runtimes are in seconds.}
	\centering
	...
	cit-Patents & 1.0 & 11.1 & 13.0 &   5.0 & N/A  & 1.3 \\
	dota-league & 1.0 & 3.8 & 1104.0 & 2.5 & \textbf{2.9} & 0.9 \\
	\multicolumn{1}{l}{}\\
	PowerGraph & BFS  & CDLP  &   LCC   &  PR &  SSSP & WCC \\ \hline
	\multicolumn{1}{l}{}\\
	GraphMat & BFS  & CDLP  &   LCC   &  PR &  SSSP & WCC \\ \hline
	cit-Patents & 7.8 & 20.6 & 11.2 &  8.4 & N/A &  7.0 \\
	dota-league & 2.2 & 19.7 & 150.1 & 2.8 & \textbf{3.8} & 3.1
	\end{tabular}

	\begin{lstlisting}
Timing results (for GraphMat SSSP on dota-league)
- Finished file read of dota-league. time: 0.9495
...
However, perusal of the log files reveals a different story. The listing contained below Table~\ref{tab:graphalytics-realworld} is the output from GraphMat itself and the execution timing (3.8 seconds) includes the time to read in the input file. However, the GraphBIG timing (2.9 seconds) does not include the time to read the dota-league file. \textbf{If the time to read in the text file was computed correctly, GraphMat would be faster than GraphBIG.} To call this a fair comparison is dubious at best when phases of execution are omitted.
...
\section{Methodology}
Our contribution strikes a balance between the highly detailed analysis such as those presented by Nai~\cite{Nai:2016:architectural} (greater depth of analysis) and Graphalytics which compares only the runtimes of a greater number of software packages and algorithms (greater breadth of analysis). This simplifies our interface which in turn allows users to perform their own analysis on their own datasets.
...
As with the graph processing systems, this approach is not specific to a particular algorithm; measuring the execution time, data structure construction time, and power consumption can be applied easily to other algorithms.
...
\begin{table}
	\centering
	\caption{This table shows averages of runtimes for the Dota-League dataset using our method using 32 threads. Each cell represents the average over 32 runs. The ``N/A'' for PowerGraph and GraphBIG exist because both of these systems read in the files and build a graph data structure simultaneously.}
	\begin{tabular}{l|c|c|c|c}
		...
		PowerGraph & N/A   & 0.36874 & 0.344    & 3.441  \\
		GAP        & 0.412 & 0.00464 & 0.267    & 0.2464 \\
		GraphMat   & 1.013 &         &          & 2.1587 \\
	\end{tabular}
	\label{tab:epg-perf}
\end{table}
...
\todo{This paragraph is out of place}
