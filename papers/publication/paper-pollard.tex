...
\maketitle
\begin{abstract}
The increasing popularity of large network analysis problems has led to the emergence of many parallel and distributed graph processing systems (one survey in 2014 identified over 80). Since then, the landscape has evolved---some packages have become inactive while others are being added. The challenge of trying to determine the best approach for a given problem is infeasible for most developers. To enable \emph{easy}, \emph{rigorous} and \emph{repeatable} comparison of parallel graph software capabilities, we have developed an approach and associated software for analyzing the performance and scalability of parallel open source graph processing systems.  We demonstrate our approach on four graph frameworks: GraphMat, GAP Benchmark Suite, GraphBIG, and PowerGraph using synthetic and real-world datasets. We analyze three algorithms: breadth first search, single source shortest paths, and PageRank and compare our results to Graphalytics. We examine previously overlooked aspects of parallel graph processing performance such as energy usage.
\end{abstract}
