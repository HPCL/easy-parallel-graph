...
From a graph algorithm user's point of view, optimizations for each system may not be apparent. For example, the Parallel Boost Graph Library (PBGL)~\cite{Gregor:2005:PBGL} provides generic implementations of their algorithms and the programmer must provide the template specializations. The optimal algorithms may differ across graphs and must be determined by the programmer. To complicate matters further, the $\Delta$-stepping implementation of the parallel Dijkstra's Algorithm for computing single source shortest paths (SSSP) is parametrized by some $\Delta > 0$ for which requires tuning for a given architecture and graph structure to achieve optimal performance \cite{Panitanarak:2014:SSSPPerf}.\todo{Will this be mentioned again?}

We aim to simplify this excess of decisions by both providing results from our experiments on real world and synthetic datasets. We take inspiration from the Graph500 benchmark \cite{Murphy:2010:Graph500} which clearly specifies every step of the breadth first search (BFS) algorithm and how it should be timed. The Graph500 can be used to fairly rank parallel graph processing platforms. We define platforms \todo{as graphalytics does? Or not.} as the combination of hardware, operating system, and graph processing library. We refer the reader to Section~[SECTION] for the platforms used in this paper. Thus, a potential user of parallel graph algorithms should have no difficulty selecting the optimal library given a wide variety of graphs and performance data available on his or her system.

We select a small number of graph processing platforms over which we attempt to homogenize all aspects of execution to truly compare performance.

%% How do we address it?
All the experiments performed use the author-provided implementations with modifications only to insert performance analysis hooks or to ensure homogeneous termination criteria; we assume the developers of each system will provide the best performing implementation. While this limits the scope of the experiments it mitigates the bias inherent in our programming skills.\todo{Should I talk about how library designers are biased?}
...
\begin{itemize}
	\item Automation of the installation\todo{This is sort of done in the repository, but not really explained in the paper}, configuration, and dataset conversion to ensure the experiments execute in the same manner. Essentially, we provide a ``level playing field'' for graph algorithms.\todo{put URL to github, make public}
...
\end{itemize}
\todo{Clarify that we're not introducing our own benchmark suite instead we're comparing existing frameworks at a greater depth.}

\section{Related Work}
\todo{Introduction sentence. How many others are there? There's a large number of qualatative. Most third party comparisons are qualitative and just describe the overview instead of actual runtimes. This isn't a level playing field.}
...
\todo{Cite the Exploring big graph computing - An empirical study from architectural perspective, but we dont do an architectural perspective. Don't care about distinctino between framework/usercode since we're just using the vendor provided implementations.}
...
We refer to a graph's \emph{scale} when describing the size of the graph. Specifically, a graph with scale $S$ has $2^S$ vertices. \todo{Say 2 to the S = 2351501351035923} The datasets are described further in Sect.~\ref{sec:datasets}. We measure strong scaling and speedup by varying the number of threads from one to the total number of threads on our server, 72.
...
The Graphalytics results in Table~\ref{tab:graphalytics} were performed on the Dota-League dataset\todo{One reviewer confused this dataset with the Kronecker.}. This dataset contains 61,670 vertices and 50,870,313 edges. This dataset comes from the Game Trace Archive\cite{Guo:2012:GTA} and is modified for Graphalytics\footnote{This dataset is available at \url{https://atlarge.ewi.tudelft.nl/graphalytics/}.}. This dataset is useful because it is both weighted and more dense than the usual real-world dataset with an average out-degree of 824.
