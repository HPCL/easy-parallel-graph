...
Table~\ref{tab:epg-perf}, in contrast, gives an overview of the performance results of running graphalytics and our approach using the same dataset [TODO: GET DOTA LEAGUE WORKING FOR EPG!]. For compactness, Table~\ref{tab:epg-perf} shows the average time across 32 roots, while Table~\ref{tab:graphalytics} shows results for a single root. Figures~\ref{fig:bfs-time} and \ref{fig:sssp-time} show the distribution of runtimes. However, even these do not give a complete picture of performance. Both of these show GraphMat to not be highly performant. However, a likely explanation is that GraphMat's underlying computation model (sparse matrix operations) paired the increased overhead of GraphMat's doubly-compressed sparse row (DSCR) graph representation is more conducive to larger-scale graphs.
...
\begin{figure}
	\centering
	\caption{The $y$-axis is logarithmic. [TODO: FIX IT SO GRAPHBIG HAS NO DSC]}
	\begin{minipage}{0.48\linewidth}
...
\begin{figure}
	...
	\begin{minipage}{0.55\linewidth}
		\includegraphics[width=\linewidth, trim=0 36pt 18pt 0, clip]{graphics/bfs_speedup.pdf}
	\end{minipage}
	\begin{minipage}{0.425\linewidth}
		\includegraphics[width=\linewidth, trim=0 36pt 18pt 0, clip]{graphics/bfs_ss.pdf}
	\end{minipage}
	\label{fig:bfs-scaling}
\end{figure}

\subsection{Power and Energy Consumption}
We use the Performance Application Programming Interface (PAPI) \cite{Browne:2000:PAPI} to gain access to Intel's Running Average Power Limit (RAPL), which provides access to a set of hardware counters measuring energy usage. PAPI provides convenient access to these values and provides average energy in nanojoules for a given time interval.

\begin{table}
	\caption{The timeframe measured here is only when the BFS is performed. Sleeping Energy refers to the power (in Watts) consumed during the Unix \texttt{sleep} function, multiplied by time. Effectively, this measures the amount of energy that would have been consumed even if nothing was running. The increase over sleep is the ratio of the first and third columns. These are all averaged over the 32 roots.}
	\centering
	\begin{tabular}{l|c|c|c|c}
			&	GAP  &    Graph500 & GraphMat & GraphBIG \\ \hline
...
\begin{figure}
	\centering
	\caption{The $y$-axis is logarithmic. GraphMat continues to run until none of the vertices' ranks change. [There is only one data point] The $y$-axis \emph{not} logarithmic. GraphMat iterations are measured differently because of the ``think like a vertex'' paradigm and runs until none of the vertices' ranks change. [Note that for figure Fig.\ref{fig:pr}, mention time per iteration]}
	\begin{minipage}{0.48\linewidth}
...
The problem of choosing a system for graph processing at large scale is far from being simple. We make our source code open source at \url{github.com/HPCL/easy-parallel-graph} and encourage further experimentation.
